...
top: int = 30
    ...
    try:
        ...
        # USA STANDINGS
        # =================================================
        standings_header_seed = [
            "tournament_id", "season_id", "cycle", "player_id",
            "matches_played", "match_points", "matches", "points",
            "mwp", "omw", "gw", "ogw", "mwp_percent",
            "game_wins", "game_losses", "game_draws", "games_played",
            "gw_percent", "omw_percent", "ogw_percent", "rank_pos"
        ]
        try:
            ws_standings = ensure_worksheet(sh, "Standings", standings_header_seed, rows=50000, cols=40)
            ensure_sheet_columns(ws_standings, standings_header_seed)
        except Exception:
            try:
        ...
        standings_header = [str(h).strip().lower() for h in standings_vals[0]]
        standings_idx = {name: i for i, name in enumerate(standings_header)}
        def getv_standings(row, *names, default=""):
            for name in names:
                i = standings_idx.get(str(name).strip().lower(), -1)
                if i >= 0 and i < len(row):
                    value = row[i]
                    if value not in (None, ""):
                        return value
        ...
        for pos, r in enumerate(table[:top], start=1):
            ...
            row_lines.append(
                ...
                f"{nome[:22]:<22} | "
                f"{r['j']:>2} | "
                f"{fmt_rank(r['score']):>7} | "
                f"{fmt_rank(r['pts']):>6} | "
                f"{fmt_rank(r['ppm']):>6} | "
                f"{fmt_rank(r['mwp_percent']):>6} | "
                f"{fmt_rank(r['omw_percent']):>6} | "
                f"{fmt_rank(r['gw_percent']):>6} | "
                f"{fmt_rank(r['ogw_percent']):>6}"
            )
        # =================================================
        # HEADER
        # =================================================
        header_lines = [
            ...
            "",
            f"{'pos':>3} | {'jogador':<22} | {'J':>2} | {'SCORE':>7} | {'PTS':>6} | {'PPM':>6} | {'MWP':>6} | {'OMW':>6} | {'GW':>6} | {'OGW':>6}",
            "-" * 106
        ]
        # =================================================
        # ENVIO EM BLOCOS
